...
public final class RedisClusterConnector extends RedisConnector {
    ...
    public RedisClusterConnector(List<Map.Entry<String, Integer>> addresses, String masterName, JedisPoolConfig poolConfig, int connectionTimeout, int soTimeout, String password, int database, String clientName, boolean ssl, SSLSocketFactory sslSocketFactory, SSLParameters sslParameters, HostnameVerifier hostnameVerifier) {
    ...
    public boolean equals(Object o) {
        ...
        RedisClusterConnector that = (RedisClusterConnector) o;
    ...
    public Map<String, Pipeline> createConnection() {
        if (masterName == null){
            ...
            getSlotHostMap(anyHost).forEach(entry -> pipelinesOfNodes.put("{" + crc16Slot.get(entry.getKey().intValue() + 1) + "}", nodeMap.get(entry.getValue()).getResource().pipelined()));
    ...
    public static RedisClusterConnector newRedisClusterConnector(List<Map.Entry<String, Integer>> addresses, String masterName, JedisPoolConfig poolConfig, int connectionTimeout, int soTimeout, String password, int database, String clientName, boolean ssl, SSLSocketFactory sslSocketFactory, SSLParameters sslParameters, HostnameVerifier hostnameVerifier) {
        return new RedisClusterConnector(addresses, masterName, poolConfig, connectionTimeout, soTimeout, password, database, clientName, ssl, sslSocketFactory, sslParameters, hostnameVerifier);
    }
    ...
    private static List<String> crc16Slot = new BufferedReader(new InputStreamReader(RedisClusterConnector.class.getResourceAsStream("/codes.txt"))).lines().collect(Collectors.toList());
