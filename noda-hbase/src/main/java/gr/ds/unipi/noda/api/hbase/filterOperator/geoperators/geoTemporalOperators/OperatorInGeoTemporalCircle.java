...
final class OperatorInGeoTemporalCircle extends GeoTemporalOperator<Circle, TemporalBounds> {
    ...
    protected Filter geometryRefactor() {
        ...
        return CircleTemporalFilter.newCircleTemporalFilter(Bytes.toBytes(getGeographicalOperator().getFieldName()), Bytes.toBytes("lon"),Bytes.toBytes("lat"),centerCoordinates, getGeographicalOperator().getGeometry().getRadius(), Bytes.toBytes(temporalNames[0]), Bytes.toBytes(temporalNames[1]), getTemporalType().getLowerBound().getTime(), getTemporalType().getUpperBound().getTime());
    }
