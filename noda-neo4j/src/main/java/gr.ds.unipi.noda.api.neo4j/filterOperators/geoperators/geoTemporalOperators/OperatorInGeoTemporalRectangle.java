...
//        long[] point2 = scalePoint(getGeographicalOperator().getGeometry().getMbr().getUpperBound().getLatitude(), getGeographicalOperator().getGeometry().getMbr().getUpperBound().getLongitude(), lowerDate.getTime(), lowerHilbertDate.getTime(), upperHilbertDate.getTime(), maxOrdinates);
        ...
        ranges.forEach(range -> {
            ...
            if(low != high) {
                sb.append("s.stHilIndex >= " + low + " AND s.stHilIndex <= " + high + " WITH s WHERE point({ srid:4326 , x: " + getGeographicalOperator().getGeometry().getLowerBound().getLongitude() + ", y: "+ getGeographicalOperator().getGeometry().getLowerBound().getLatitude() +" }) <= s." + getGeographicalOperator().getFieldName() + " <= point({ srid: 4326 , x: " + getGeographicalOperator().getGeometry().getUpperBound().getLongitude() + ", y: "+ getGeographicalOperator().getGeometry().getUpperBound().getLatitude() + " }) AND " + lowerDate.getTime() + " <= s." + getTemporalFieldName()  + " <= " + upperDate.getTime() );

            } else {
                sb.append("s.stHilIndex = " + low + " WITH s WHERE point({ srid:4326 , x: " + getGeographicalOperator().getGeometry().getLowerBound().getLongitude() + ", y: "+ getGeographicalOperator().getGeometry().getLowerBound().getLatitude() +" }) <= s." + getGeographicalOperator().getFieldName() + " <= point({ srid: 4326 , x: " + getGeographicalOperator().getGeometry().getUpperBound().getLongitude() + ", y: "+ getGeographicalOperator().getGeometry().getUpperBound().getLatitude() + " }) AND " + lowerDate.getTime() + " <= s." + getTemporalFieldName()  + " <= " + upperDate.getTime() );
