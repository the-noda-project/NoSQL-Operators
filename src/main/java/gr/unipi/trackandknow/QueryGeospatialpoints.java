    ...
    public static void main(String[] args) throws ParseException {
        ...
        if(Integer.valueOf(args[0])==0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                noSqlDbSystem3.operateOn("geoPoints").filter(FilterOperators.inGeoBox("location", Coordinates.newCoordinates(23.6266, 37.9262), Coordinates.newCoordinates(23.6682, 37.9477)),gte("date",minDate),lte("date",maxDate)).count();
        ...
        if(Integer.valueOf(args[0])==1) {
            ...
            for (int i = 0; i < 3; i++) {
                long start = System.currentTimeMillis();
                noSqlDbSystem2.operateOn("geoPoints").filter(FilterOperators.inGeoBox("location", Coordinates.newCoordinates(23.5500, 37.9262), Coordinates.newCoordinates(23.95, 38.30)),gte("date",minDate),lte("date",maxDate)).count();
                System.out.println(((System.currentTimeMillis() - start) / 1000f) + " sec");
            }
        ...
        if(Integer.valueOf(args[0])==2) {
            ...
            for (int i = 0; i < 3; i++) {
                long start = System.currentTimeMillis();
                noSqlDbSystem1.operateOn("geoPoints").filter(FilterOperators.inGeoBox("location", Coordinates.newCoordinates(23.5500, 37.9262), Coordinates.newCoordinates(23.95, 38.30)),gte("date",minDate),lte("date",maxDate)).count();
                System.out.println(((System.currentTimeMillis() - start) / 1000f) + " sec");
            }
